...
def reqcreate(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            markets = StockList.objects.all().filter(
                stockattribute=stock).values_list('stockprice', flat=True)
            print(markets[0])
            cap = markets[0]*0.1
                ...
                if not (priceperstock < lowercap or priceperstock > uppercap):
                    if action == 'buy':
                        ...
                        if stock == 'RBI':
                            ...
                            if (numberofstock <= stock_request_sender.ADANIENT):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.BOSCHLTD):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.TCS):
                                ...
                                messages.error(
                                    request, f'Insufficient Stock holdings!!')
                        elif stock == 'TATAMOTORS':
                            if (numberofstock <= stock_request_sender.TATAMOTORS):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.ICICIPRULI):
                                ...
                                messages.error(
                                    request, f'Insufficient Stock holdings!!')
                        elif stock == 'HINDUNILVR':
                            if (numberofstock <= stock_request_sender.HINDUNILVR):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.GLENMARK):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.LUPIN):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                            ...
                            if (numberofstock <= stock_request_sender.ONGC):
                                ...
                                messages.error(
                                    request, f'Insufficient Stock holdings!!')
                        elif stock == 'NETWORK18':
                            if (numberofstock <= stock_request_sender.NETWORK18):
                                request_trade = tradereq.objects.create(
                                    sender=sender,
                                    receiver=receiver, action=action,
                                    stock=stock,
                    ...
                    messages.error(
                        request, f'You cannot Buy/Sell at price greater/lower than 10% of market price!')
            else:
...
def received_request(request):
    user = request.user
    requests_pending = tradereq.objects.order_by(
        '-id').filter(receiver=user, is_active=True)
...
def sent_request(request):
    user = request.user
    sent_pending = tradereq.objects.order_by(
        '-id').filter(sender=user, is_active=True)
...
def all_history(request):
    ...
    comb_query = tradereq.objects.filter(
        sender=user) | tradereq.objects.filter(receiver=user)
